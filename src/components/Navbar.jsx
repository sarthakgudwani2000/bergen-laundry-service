...
import { BRAND, IMAGES, SCHEDULE_ORDER_URL, SINGH_SITE_URL, SINGH_SERVICES_URL, SINGH_ABOUT_URL, SINGH_CONTACT_URL, BERGEN_SITE_URL, NEW_BRIDGE_SITE_URL } from "@/lib/brand";
...
export default function Navbar() {
  ...
  const links = [
    { href: SINGH_SITE_URL, label: "Home", short: "Home" },
    { href: SINGH_SERVICES_URL, label: "Services", short: "Services" },
    { href: BERGEN_SITE_URL, label: BRAND.pickup, short: "Bergen", active: pathname === BERGEN_PICKUP_PATH },
    { href: NEW_BRIDGE_SITE_URL, label: "New Bridge Laundromat", short: "New Bridge", active: false },
    { href: SINGH_ABOUT_URL, label: "About Us", short: "About" },
    { href: SINGH_CONTACT_URL, label: "Contact Us", short: "Contact" },
  ];
  const isBergenPickupPage = pathname === BERGEN_PICKUP_PATH;
  const isNewBridgePage = false;
  ...
  // ── DEBUG LOGGING ──────────────────────────────────────────────
  useEffect(() => {
    console.group("%c[Bergen Navbar] location update", "color:#1d4ed8;font-weight:bold");
        ...
        <div className="container-pad">
          <div className="flex h-[4.25rem] md:h-[4.5rem] items-center justify-between gap-4 lg:gap-8">
            <Link
              ...
              aria-label={`${BRAND.pickup} — ${BRAND.parent} home`}
            >
              <div
                ...
                {links.map((l) => (
                  <li key={l.to ?? l.href}>
                    {l.href ? (
                      <a
                        href={l.href}
                        onClick={() => console.log("%c[Bergen Navbar] tab click → EXTERNAL", "color:#15803d;font-weight:bold", { label: l.label, href: l.href })}
                      ...
                      <NavLink
                        ...
                        onClick={() => console.log("%c[Bergen Navbar] tab click → INTERNAL", "color:#9333ea;font-weight:bold", { label: l.label, to: l.to })}
                        className={({ isActive }) =>
                          [
